...
var sql = module.exports = {
	...
	deleteOneUser: async function(id){
		...
		// deleting dependicias
		str = `delete from tb_usu_module_permission where mde_usu_cd = ${id}`;
	...
	setPermissions: async function (data){
		...
		if(typeof data.permission == 'string'){
			...
			str += `VALUES (${data.id}, ${parseInt(data.permission)});`;
			...
			for (let index = 0; index < data.permission.length; index++) {
				...
				str += `INSERT INTO tb_usu_module_permission `;
				str += `( mde_usu_cd, mde_mod_cd ) `;
				str += `VALUES (${data.id}, ${parseInt(data.permission[index])});`;
